...
function scurf_referral_published_post_list_only_shortcode($atts) {
    ...
    // Get the published post records for the user.
    global $wpdb;
    $table_name = $wpdb->prefix . 'referral_history';
    /* $results = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE user_id = %d AND type = '$tl_publish_post' AND points >= 1 ORDER BY created_at DESC", $user_id)); */
...
add_shortcode('referral_published_post_list_only', 'scurf_referral_published_post_list_only_shortcode');
?>